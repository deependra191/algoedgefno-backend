...
17. **No magic strings** — use named constants for: (a) values used in multiple places, (b) values with branching logic depending on them, (c) sentinel values, (d) cryptic external identifiers — vendor CSV column names, protocol codes, external API field names — even if used only once. The test for (d): if a reader must look up what the string means, it needs a constant with a readable name. Env var key names defined once in `config.go` are fine inline. "Used only once" is not sufficient justification to inline an opaque external identifier. (e) **Production contract strings** — API/error response codes (the `{"error":"…"}` body values), JSON field and structured-log attribute keys, JWT/claim names, external SQLSTATE/vendor codes, and status/enum values — must be named constants, defined once and reused. Inline literals remain fine for: human-readable log/error message *prose* (including `fmt.Errorf` wrapping text and `slog` message strings, but NOT their attribute keys), SQL query bodies, and CLI help/usage text. In **test files** the rule is relaxed — inline literals are fine unless a value is reused, contract-like, or central to a fixture.
