...
--
-- Singleton enforcement: the boolean primary key constrained to TRUE means at most one row can exist.
-- The CHECK on identity rejects any value other than the two recognised environments.
CREATE TABLE IF NOT EXISTS environment_identity (
    id         BOOLEAN     PRIMARY KEY DEFAULT TRUE CHECK (id),
    identity   VARCHAR(50) NOT NULL CHECK (identity IN ('production', 'staging')),
