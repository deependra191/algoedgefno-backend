...
19. **Named request and response structs for every handler** — never use inline anonymous structs or `gin.H` maps for request parsing or responses. Define named structs (e.g. `loginRequest`, `authResponse`) in the same handler file. This makes the API contract with Android explicit and prevents silent key-name mismatches. See `internal/handlers/auth.go` as the pattern to follow.
