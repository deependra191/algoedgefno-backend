...
20. **Three struct layers for persisted resources** — every DB-backed resource has (1) a pgx scan target in `internal/entities/`, (2) a domain object in `internal/models/` with `FromXEntity`/`ToEntity` mappers, and (3) a handler-local response DTO. Entities and domain models carry NO `json:` tags — only response DTOs do. Storage returns `*entities.X`; services convert to `*models.X`; handlers convert to local DTOs before `c.JSON`. This prevents a DB column rename from silently changing the Android API contract and makes credential leakage (e.g. a forgotten `json:"-"`) impossible by construction.
21. **Engine types carry no JSON tags** — `internal/engine/` structs have no `json:` annotations. Handlers define response DTOs for engine output (e.g. `tradeResponse`, `backtestResponse`) and map engine types to DTOs before serializing.
